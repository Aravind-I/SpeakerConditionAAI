...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov 19 18:21:34 2019
...
def Get_Wav_EMA_PerFile(EMA_file,Wav_file,F):
    ...
    ## X-vector Embeddings
    SPK_emd=np.loadtxt(XvectorPath+Wav_file[:-4]+'.txt')
    return Ema[EBegin:EEnd,:], MFCC_G[EBegin:EEnd,:n_mfcc],TimeStepsTrack,SPK_emd # with out silence
...
ExcludeSet1=['Ankur', 'Anand_S' , 'Prasad','Ankur_C', 'GokulS', 'Monika', 'SriRamya','Harshini','Jisha','Anisha']
...
for ss in np.arange(0,len(Trainsubs)):
    ...
    for i in np.arange(0,460):
        if  (((i+F)%10)==0):# Test
            E_t,M_t,TT,se=Get_Wav_EMA_PerFile(EMAfiles[i],Wavfiles[i],i)
#            timesteps,fvdim=np.shape(E_t)
#            spk_emb=np.ones((timesteps,512),order='C')*se
#            spk_emb=spk_emb[np.newaxis,:,:]
...
E_valseq=pad_sequences(E_valseq, padding='post',maxlen=TT_max,dtype='float')
...
E_trainseq=pad_sequences(E_trainseq, padding='post',maxlen=TT_max,dtype='float')
...
#OutFileName='Xvec_Embd_AAI_Batch50_UnseenVal_LSTMunits_256_' #
OutFileName='Spk_Embd_AAI_'+'Batch'+ str(BatchSize)+'_'+'AtreyeeS'+'_LSTMunits_'+str(NoUnits)+'_'
fName=OutFileName
#print('..fitting model')
...
#history=model.fit([X_trainseq,E_trainseq],Youttrain,validation_data=([X_valseq,E_valseq],Youtval),nb_epoch=NoEpoch, batch_size=BatchSize,verbose=1,shuffle=True,callbacks=[checkpointer,checkpointer1,earlystopper])
...
#scipy.io.savemat(OutDir+fName+'_10_UnSeen_Results.mat',{'RMSE':TSrmse,'Corr':TSCC})

fName=fName+'_20_Seen_'
