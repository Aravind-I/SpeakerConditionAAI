...
for ss in np.arange(0,len(Trainsubs)):
    ...
    for i in np.arange(0,460):
        if  (((i+F)%10)==0):# Test
            ...
            spk_emb=np.ones((timesteps,1),order='C')*ss
            #W_t=W_t#[np.newaxis,:,:,:]
            E_t=E_t#[np.newaxis,:,:]
    ...
    OutFileName='FT_Spk_Embd_AAI_'+'Batch'+ str(BatchSize)+'_'+Sub+'_LSTMunits_'+str(NoUnits)+'_'
    fName=OutFileName
    print('..loading model')
    model=load_model('Pooled_AAI_Model/'+'Spk_Embd_AAI_'+'Batch'+ str(BatchSize)+'_'+'AtreyeeS'+'_LSTMunits_'+str(NoUnits)+'__'+'.h5')
    print('..fitting model')
    ...
    import gc
    gc.collect()
    scipy.io.savemat(OutDir+fName+'_Results.mat',{'RMSE':TSrmse,'Corr':TSCC})
