...
for ss in np.arange(0,len(Valsubs)):
    Sub=Valsubs[ss]#'Anand_S'
    print(Sub)
    WavDir=RootDir+'DataBase/'+Sub+'/Neutral/WavClean/';
    EmaDir=RootDir+'DataBase/'+Sub+'/Neutral/EmaClean/';
    BeginEndDir=RootDir+'/StartStopMat/'+Sub+'/';
...
#print '..compiling model'
# model.compile(loss=newDict['loss'], optimizer=SGD(lr=.01, momentum=0.3,decay=1e-6, nesterov=True));
#model.compile(loss=newDict['loss'], optimizer=Nadam(lr=0.002, beta_1=0.9, beta_2=0.999, epsilon=1e-08, schedule_decay=0.004))
#M1.compile(loss='mse', optimizer=Adam(lr=0.001, beta_1=0.9, beta_2=0.999, epsilon=1e-08))
model = Sequential()
model.add(LSTM(150, return_sequences=True,activation='sigmoid',input_shape=(None, inputDim)))  # returns a sequence of vectors of dimension 32
...
#model.add(Bidirectional(LSTM(150, return_sequences=True,activation='sigmoid')))
#model.add(Bidirectional(LSTM(150, return_sequences=True,activation='sigmoid')))
model.add(TimeDistributed(Dense(100, activation='tanh')))
model.add(GlobalAveragePooling1D())
model.add(Dense(len(Trainsubs), activation='softmax'))
...
OutFileName='Pool_spkID_'+'Batch'+ str(BatchSize)+'_'+Sub+'_LSTMunits_'+str(150)+'_'
fName=OutFileName
print('..fitting model')
...
history=model.fit(X_trainseq,to_categorical(Youttrain,len(Trainsubs)),validation_data=(X_valseq,to_categorical(Youtval,len(Trainsubs))),nb_epoch=NoEpoch, batch_size=BatchSize,verbose=1,shuffle=True,callbacks=[checkpointer,checkpointer1,earlystopper])
...
for ss in np.arange(0,len(Trainsubs)):
    ...
    tCC=[]
    trMSE=[]
    for i in np.arange(0,len(X_testseq)):
        s_in=X_testseq[i]
        ...
        tCC.append(numpy.argmax(val))# iCC)
        trMSE.append(ss)
    ...
    print('Pred:'+str(val)+'org'+str(ss))
    print(tCC)
    TSrmse.append(trMSE)#,axis=0)
    TSCC.append(tCC)#,axis=0)


# In[68]:


scipy.io.savemat(OutDir+OutFileName+'_Results.mat',{'Pred':TSCC,'Org':TSrmse})
